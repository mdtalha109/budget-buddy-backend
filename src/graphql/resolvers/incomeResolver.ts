...
const incomeResolvers: IResolvers = {
    ...
    Mutation: {
        addIncome: withAuth(async (_, { description, amount, date, category }, context: Context) => {
            try {
                return await incomeService.addIncome(context.userId!, description, amount, new Date(date), category);
            } catch (error) {
                handleError(error);
            }
        }),
        updateIncome: withAuth(async (_, { id, description, amount, date, category }, context: Context) => {
            try {
                return await incomeService.updateIncome(context.userId!, Number(id), description, amount, date ? new Date(date) : undefined, category);
            } catch (error) {
                handleError(error);
            }
        }),
